...
export function UploadDocumentsForm() {
  ...
  const ingest = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await api.post("/retriveal/ingest", formData, {
        headers: {
  ...
  return (
    <form
      onSubmit={ingest}
      ref={formRef}
      className="flex flex-col w-full flex-1 p-4 gap-2.5 min-w-[200px]"
    >
      <input
        type="file"
        id="document-input"
      ...
      <button
        type="submit"
        className="shrink-0 p-2.5 bg-[#7159c1] rounded w-full hover:brightness-110 transition-all"
      >
        <div
          role="status"
