...
export function UploadDocumentsForm() {
  ...
  const [formState, setFormState] = useState(window.innerWidth > 768);
  ...
  const ingest = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = sessionStorage.getItem("ragChat@sessiorId")
        ? await api.post(
        ...
        : await api.post("/retriveal/ingest", formData, {
            headers: {
              "Content-Type": "multipart/form-data",
            },
          });
      ...
      if (response.status === 200) {
        ...
        if (!sessionStorage.getItem("ragChat@sessiorId"))
          sessionStorage.setItem("ragChat@sessiorId", response.data.uuid);
